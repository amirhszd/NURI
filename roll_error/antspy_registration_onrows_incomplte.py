...
def main(swir_hdr, mica_hdr):
    ...
    waterfall_rows = swir_arr[..., -2].squeeze()
    ...
    a = np.arange(1, np.max(waterfall_rows) + 1)
    chunks = np.split(a, np.arange(10, len(a), 10))
    for chunk in chunks:
        to_uint8 = lambda x: ((x - np.nanmin(x)) / (np.nanmax(x) - np.nanmin(x)) * 255).astype(np.uint8)
        list_of_rows = [waterfall_rows == i for i in chunk]
        mask_bool = np.logical_or.reduce(list_of_rows)
        ...
        # grab the patch
        # antspy does not support boolean
        mask_bin_patch = (mask_bool[min_row:max_row + 1, min_col:max_col + 1])
        # mask_bool_patch_ants = ants.from_numpy(mask_bin_patch, is_rgb=False, has_components=False)

        swir_patch = swir_image[min_row:max_row + 1, min_col:max_col + 1]
        # swir_patch[swir_patch == 0] = np.nan
        # swir_patch = to_uint8(swir_patch)
        swir_image_uint8_ants = ants.from_numpy(swir_patch, is_rgb=False, has_components=False)

        mica_patch = mica_image[min_row:max_row + 1, min_col:max_col + 1]
        # mica_patch[mica_patch == 0] = np.nan
        # mica_patch = to_uint8(mica_patch)
        mica_image_uint8_ants = ants.from_numpy(mica_patch, is_rgb=False, has_components=False)
        ...
        for i in [10,40,90]:
            swir_band_ants = ants.from_numpy(np.array(swir_arr[min_row:max_row + 1, min_col:max_col + 1,i].squeeze()), is_rgb=False, has_components=False)
